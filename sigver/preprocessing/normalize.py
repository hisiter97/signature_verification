...
def preprocess_signature(img: np.ndarray,
                         canvas_size: Tuple[int, int] = (400, 574),
                         img_size: Tuple[int, int] =(170, 242),
                         input_size: Tuple[int, int] =(150, 220),
                         resize_size: Tuple[int, int] = (200, 400)) -> np.ndarray:
    ...
    #     img_resized = img
    # else:
    width_ratio = float(w) / new_w
    height_ratio = float(h) / new_h
    if h > w:
        resize_height = new_h
    ...
    img_resized = cv2.resize(img, (resize_width, resize_height), interpolation=cv2.BORDER_CONSTANT)
        # cv2.imwrite("")
...
def normalize_image(img: np.ndarray,
    ...
    if threshold == 0:
        #
        binarized_image = blurred_image
        r, c = np.where(binarized_image == 255)
    else:
        #Find the center of mass

        binarized_image = blurred_image > threshold + threshold*0.2
    ...
    normalized_image = np.ones((max_rows, max_cols), dtype=np.uint16) * 255
