...
def process_dataset(dataset: IterableDataset,
    ...
    preprocess_fn = functools.partial(preprocess_signature,
                                      resize_size=dataset.resize_size,
    ...
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    # for ind, (img, path) in enumerate(zip(x[:100], used_files[:100])):
    #     print(img.shape)
    #     cv2.imwrite(os.path.join(save_path,  str(y[ind]) + "_" + os.path.basename(path)), np.transpose(img, (1,2,0)))
    #
    # return
    np.savez(save_path,
             x=x,
