...
def process_dataset(dataset: IterableDataset,
    ...
    # check
    # for img in x:
    #     cv2.imwrite("check/" + uuid.uuid4() + ".jpg", img)
    #     return
...
def process_dataset_images(dataset: IterableDataset,
    ...
    H, W = img_size
    max_signatures = len(users) * (dataset.genuine_per_user + dataset.skilled_per_user + dataset.simple_per_user + dataset.opposite_per_user)
    ...
    for i, user in enumerate(tqdm(users)):
        ...
        if len(user_forg_data) > 0:
            ...
            indexes = slice(N, N + new_img_count)
            x[indexes] = forg_imgs
            yforg[indexes] = 1  # Simple forgeries
            y[indexes] = i
            used_files += forg_filenames
            N += new_img_count

        # Opposite hand forgeries
        user_forg_data = [(preprocess_fn(img), filename) for (img, filename) in dataset.iter_opposite_forgery(user)]
        if len(user_forg_data) > 0:
            forg_imgs, forg_filenames = zip(*user_forg_data)
            new_img_count = len(forg_imgs)
            ...
            indexes = slice(N, N + new_img_count)
            x[indexes] = forg_imgs
            yforg[indexes] = 1  # Opposite hand forgeries
            y[indexes] = i
            used_files += forg_filenames
            N += new_img_count
...
# process_dataset("/home/hisiter/working/HTI/Signature_Verification/data/UTSig", save_path=".", img_size=(150,220))
